...
public class CanteenControllerTest {
    ...
    public void testGetTodayMeals_ReturnsOkWithMeals() throws Exception {
        ...
        int weekNumber = today.get(WeekFields.of(Locale.getDefault()).weekOfWeekBasedYear()); String weekStr = String.format("%02d", weekNumber);
        final var expectedWeek = createTestData(today, weekNumber, year);
            // Mock the API response
        String canteenName = "mensa-garching";
        String expectedUrl = "https://tum-dev.github.io/eat-api/" + canteenName + "/" + year + "/" + weekStr + ".json"; when(restTemplate.getForObject(eq(expectedUrl), eq(Week.class))).thenReturn(expectedWeek);
        // Act
        ...
        var actualDish1 = actualTodayDishes.getFirst(); assertThat(actualDish1.name()).isEqualTo("Vegetarian Pasta"); assertThat(actualDish1.dish_type()).isEqualTo("Main Dish"); var actualDish2 = actualTodayDishes.get(1); assertThat(actualDish2.name()).isEqualTo("Salad"); assertThat(actualDish2.dish_type()).isEqualTo("Side Dish");
    }
